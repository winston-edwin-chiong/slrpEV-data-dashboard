...
class PlotDaily:
    ...
    def plot_today_forecast(cls, fig: go.Figure, df: pd.DataFrame) -> go.Figure:
        ...
        if not df.loc[df.index == datetime.now(pytz.timezone('US/Pacific')).strftime("%Y-%m-%d")].empty:
            ...
            peak = df.loc[df.index == datetime.now(pytz.timezone('US/Pacific')).strftime("%Y-%m-%d")]["peak_power_kW_predictions"].iloc[0]
...
class PlotSchedVsReg:     
    ...
    def plot_sched_vs_reg(cls, df: pd.DataFrame, theme=None) -> go.Figure():
        ...
        fig.update_traces(hovertemplate="Scheduled ¢/Hr: %{y}<br>Regular ¢/Hr: %{x}<br>Count: %{marker.size:,}<br>Choice: %{customdata}<extra></extra>")
        fig.update_layout(
