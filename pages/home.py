...
def calcuate_stats_change(kwh_today, num_users, peak_power, dailydemand, raw_data, monthlydemand):
    ...
    yesterday = pd.to_datetime(
        pd.Timestamp.now().normalize() - pd.DateOffset(days=1))
    yesterday_kwh = dailydemand.loc[dailydemand.index ==
                                    yesterday]["energy_demand_kWh"].iloc[0]
    kwh_change = calculate_pct_change(kwh_today, yesterday_kwh)

    # calculate number of users change
    yesterday_users = raw_data.loc[pd.to_datetime(
        raw_data["connectTime"]).dt.normalize() == yesterday]["userId"].nunique()
    num_users_change = num_users - yesterday_users

    # calculate monthly peak power change
    last_month_peak_power = monthlydemand.iloc[[-2]
                                               ]["peak_power_W"].iloc[0] / 1000
    peak_power_change = calculate_pct_change(peak_power, last_month_peak_power)

    return (
        [
            html.I(
                className="bi bi-arrow-up me-1" if kwh_change > 0 else (
                    "bi bi-arrow-down me-1" if kwh_change else "bi bi-arrow-right me-1"),
                style={"color": "#58c21e" if kwh_change > 0 else (
                    "#c21e58" if kwh_change else "#55595c")}
            ),
            f"{round(kwh_change, 2):+} % since yesterday!" if kwh_change else "No change since yesterday!"
        ],
        [
            html.I(
                className="bi bi-arrow-up me-1" if num_users_change > 0 else (
                    "bi bi-arrow-down me-1" if num_users_change else "bi bi-arrow-right me-1"),
                style={"color": "#58c21e" if num_users_change > 0 else (
                    "#c21e58" if num_users_change else "#55595c")}
            ),
            f"{num_users_change:+} users since yesterday!" if num_users_change else "No change since yesterday!"
        ],
        [
            html.I(
                className="bi bi-arrow-up me-1" if peak_power_change > 0 else (
                    "bi bi-arrow-down me-1" if peak_power_change else "bi bi-arrow-right me-1"),
                style={"color": "#58c21e" if peak_power_change > 0 else (
                    "#c21e58" if peak_power_change else "#55595c")}
            ),
        ...
        dbc.Container([
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader([
                            html.H5("Cumulative Energy Delivered",
                ...
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader([
                            html.H5("Energy Delivered Today",
                                    className="card-title"),
                        ]),
                        dbc.CardBody([
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader([
                            html.H5("Cumulative E-Miles Delivered",
                                    className="card-title"),
                        ]),
                        dbc.CardBody([
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader([
                            html.H5("Cumulative Number of Sessions",
                                    className="card-title"),
                        ]),
                        dbc.CardBody([
...
def update_today_homepage_cards(n):
    ...
    if len(today) == 0:
        ...
        today = today[["dcosId", "cumEnergy_Wh", "vehicle_model"]
...
def update_cum_homepage_cards(n):
    ...
    cum_sessions_stats = [html.I(className="bi bi-ev-station me-1"),
    ...
    cum_kwh_stats = [html.I(className="bi bi-lightning-charge-fill me-1"),
    ...
    cum_emiles_delivered = f'{cum_emiles_float:,}' + \
        " Mi"  # 290 Wh/mile conversion rate
    cum_emiles_stats = [html.I(className="bi bi-geo-alt-fill me-1"),
